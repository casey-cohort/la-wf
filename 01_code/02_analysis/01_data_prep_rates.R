...
pacman::p_load(tidyverse, readr, tidyr, purrr, lubridate, MMWRweek, here, arrow, readxl)
...
#-------------------------------
# load data
df_counts <- read_csv(paste0(path_onedrive, "01_data/01_raw/ed_ipt_dat/08-08-2025/ENC_EXP_DAILY_08082025.csv")) %>% 
...
# load denoms 
denoms <- read_excel(paste0(path_onedrive, "01_data/01_raw/ed_ipt_dat/30-12-2025/denoms_enc_exp_daily_30122025.xlsx")) %>%
  mutate(exposure_category = str_replace_all(exposure_category, ",.*", "")) %>%
...
df_temp <- df_counts %>%
  ...
  mutate(rate_enc = num_enc / N, 
         rate_enc_cardio = num_enc_cardio / N,
         rate_enc_resp = num_enc_resp / N,
         rate_enc_neuro = num_enc_neuro / N,
         rate_enc_injury = num_enc_injury / N)

# drop num_enc and denom
df_temp <- df_temp %>%
  select(-c(num_enc, num_enc_cardio, num_enc_resp, num_enc_neuro, num_enc_injury, N, year, month))
...
df <- df_temp %>%
  ...
  summarise(across(c(rate_enc, rate_enc_cardio, rate_enc_resp, rate_enc_neuro, rate_enc_injury), mean, na.rm = TRUE)) %>%
  mutate(encounter_dt = as.Date(encounter_dt),
...
# 
out_df <- df %>%
  select(enc_type, exposure_category, encounter_dt, 
          rate_enc, rate_enc_cardio, rate_enc_resp, rate_enc_neuro, rate_enc_injury,
...
df_train_test <- out_df %>%
  ...
  select(-c('encounter_dt')) %>%
    ...
    mutate(across(where(is.numeric), ~ if(cur_column() %in% c("rate_enc", "rate_enc_cardio", "rate_enc_resp", "rate_enc_neuro", "rate_enc_injury")) .x else as.integer(.x))) %>%
    arrange(date)


dir.create(paste0(path_onedrive, "01_data/02_processed/train_test/", Sys.Date()), recursive = TRUE, showWarnings = FALSE)
write_parquet(df_train_test, paste0(path_onedrive, paste0( "01_data/02_processed/train_test/", Sys.Date(), "/df-train-test_rates.parquet")))
...
write_parquet(df_all_cases, paste0(path_onedrive, "01_data/02_processed/train_test/", Sys.Date(), "/df-predict-rates.parquet"))
